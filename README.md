...
SamWinOptimize 是对 `ZyperWinOptimize 4.1` 的现代化重构：从固定尺寸、脚本混合执行的 WinForms 工具，升级为基于 **.NET 10 WinForms** 的本地优先 Windows 操作控制台，采用深色科技玻璃视觉系统。
...
- **组件化界面**：使用第一方 WinForms 自绘控件与统一设计令牌，不再依赖 AntdUI；表面、按钮、导航和背景统一到深色玻璃质感。
- **响应式桌面布局**：默认窗口 1520×960，最小窗口 1240×800；筛选栏、任务卡、页面内容会随窗口宽度调整。
